...
#define DWC3_ALIGN_FRAME(d, n)                                                 \
	(((d)->frame_number + ((d)->interval * (n))) & ~((d)->interval - 1))
...
static int __dwc3_gadget_ep_queue(struct dwc3_ep *dep, struct dwc3_request *req)
{
	 ...
	 */
	if ((dep->flags & DWC3_EP_END_TRANSFER_PENDING) ||
	    (dep->flags & DWC3_EP_WEDGE) || (dep->flags & DWC3_EP_STALL)) {
...
static int dwc3_gadget_pullup(struct usb_gadget *g, int is_on)
{
	...
	if (((dwc->dr_mode > USB_DR_MODE_HOST) && !dwc->vbus_active) ||
	    !dwc->gadget_driver || (dwc->err_evt_seen && dwc->softconnect)) {
		/*
...
static void dwc3_gadget_free_endpoints(struct dwc3 *dwc)
{
	...
	for (epnum = 0; epnum < DWC3_ENDPOINTS_NUM; epnum++) {
		...
		debugfs_remove_recursive(debugfs_lookup(
			dep->name, debugfs_lookup(dev_name(dep->dwc->dev),
						  usb_debug_root)));
		kfree(dep);
	}
}
...
static void dwc3_gadget_conndone_interrupt(struct dwc3 *dwc)
{
	...
	if ((dwc->revision > DWC3_REVISION_194A) &&
	    !dwc->usb2_gadget_lpm_disable && (speed != DWC3_DSTS_SUPERSPEED) &&
